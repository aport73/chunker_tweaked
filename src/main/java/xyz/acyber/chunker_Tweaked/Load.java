...
public class Load {
	...
	public void state(JavaPlugin plugin, PreGenerationTask task) {
		...
		if (dataFile.exists()) {
			try {
				...
				if (!lines.isEmpty()) {
					...
					if (parts.length == 7) {
						...
						ConsoleColorUtils.logPlain("Successfully loaded " + processedChunks + " processed chunks for " + task.world.getName());
					} else {
				...
				e.printStackTrace();
				ConsoleColorUtils.exceptionMsg("Failed to load processed chunks for " + task.world.getName() + ": " + e.getMessage());
			...
			task.chunkIterator.reset();
			task.totalChunksProcessed.reset();
			ConsoleColorUtils.logPlain("No pre-generator data found for " + task.world.getName() + ". Starting fresh.");
		}
	}
}